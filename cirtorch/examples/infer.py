...
whitening_names = ['retrieval-SfM-30k', 'retrieval-SfM-120k', 'colon']
...
parser.add_argument('--whitening', '-w', metavar='WHITENING', default=None, choices=whitening_names,
                    help='dataset used to learn whitening for testing: ' + 
                        ...
                        ' (default: None)')
parser.add_argument('--save_result', help='save the confusion matrix as this file.')
parser.add_argument('--colon_image_dir', help='directory containing testing colon images')
parser.add_argument('--save_Lw_as', help='save the learned m and P in this file')
parser.add_argument('--load_Lw', help='load Lw (m and P) from this file (.pkl)')
# GPU ID
...
def main():
    ...
    if args.whitening is not None:
        start = time.time()
        if args.load_Lw:
            with open(args.load_Lw, 'rb') as handle:
                Lw = pickle.load(handle)
        elif 'Lw' in net.meta and args.whitening in net.meta['Lw']:
            ...
            wvecs = wvecs.numpy()
            m, P = whitenlearn(wvecs, db['qidxs'][:MAX_NUM_QURIES], db['pidxs'][:MAX_NUM_QURIES])
            Lw = {'m': m, 'P': P}
            if args.save_Lw_as:
                with open(args.save_Lw_as, 'wb') as handle:
                    pickle.dump(Lw, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    for dataset in datasets: 
        ...
        if args.datasets.startswith('colon'):
            images = glob.glob(os.path.join(args.colon_image_dir, '*'))
            images.sort()
            qimages = images
        else:
        ...
        if not args.datasets.startswith('colon'):
        ...
        scores *= np.tri(*scores.shape)
        print(scores)
        print('>> {}: elapsed time: {}'.format(dataset, htime(time.time()-start)))
        # img = Image.fromarray((scores*255).astype(np.uint8))
        # img.save(args.save_result)
        with open(args.save_result, 'wb') as handle:
