...
from cirtorch.examples.test import PRETRAINED

from tensorboard_logger import configure, log_value

training_dataset_names = ['retrieval-SfM-120k', 'colon']
test_datasets_names = ['oxford5k,paris6k', 'roxford5k,rparis6k', 'oxford5k,paris6k,roxford5k,rparis6k', 'colon']
test_whiten_names = ['retrieval-SfM-30k', 'retrieval-SfM-120k', 'colon']
...
parser.add_argument('--network-path', help='path to the network that is trained on cnnimageretrieval, we do further fine-tuning.',
                    default=None)
...
def main():
    ...
    configure(os.path.join(args.directory, 'log'))
    ...
    mean = None
    std = None
    # if args.training_dataset.startswith('colon'):
    #     mean = [0.70352508, 0.42265634, 0.31021822]
    #     std = [0.24151333, 0.19284668, 0.1609257]
    if args.network_path:
        ...
        print(">> Whitening {}".format(args.whitening))
        if mean:
    ...
    print('Using normalization mean={} std={}'.format(model.meta['mean'], model.meta['std']))
    transform = transforms.Compose([
        transforms.ToTensor(),
        normalize,
    ])
    transform_train = transforms.Compose([
        transforms.RandomResizedCropForRectangle(scale=(0.49, 1), ratio=(0.8,1.25)),
    ...
    test_map = test(args.test_datasets, model)
    log_value('test/mAP', test_map, 0)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        log_value('train/loss', loss, epoch + 1)

        # evaluate on validation set
        if args.val:
            loss = validate(val_loader, model, criterion, epoch)
            log_value('val/loss', loss, epoch + 1)

        # evaluate on test datasets
        test_map = test(args.test_datasets, model)
        log_value('test/mAP', test_map, epoch + 1)
...
def test(datasets, net):
    ...
    if args.test_whiten:
        ...
        if args.datasets.startswith('colon'):
            db_fn = os.path.join(db_root, '{}.pkl'.format(args.whitening))
        else:
            db_fn = os.path.join(db_root, '{}-whiten.pkl'.format(args.whitening))
        with open(db_fn, 'rb') as f:
            db = pickle.load(f)
        if args.whitening.startswith('colon'):
            images = db['image_paths']
        else:
    ...
    for dataset in datasets: 
        ...
        res = compute_map_and_print(dataset, ranks, cfg['gnd'])
        ...
        if Lw is not None:
            ...
            ranks = np.argsort(-scores, axis=0)
            res = compute_map_and_print(dataset + ' + whiten', ranks, cfg['gnd'])
