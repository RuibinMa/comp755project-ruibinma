...
def compute_ap(ranks, nres):
    ...
    for j in np.arange(nimgranks):
        ...
        if j == 0:
            ...
            precision_0 = float(j) / (ranks[j-1] + 1)

        precision_1 = float(j + 1) / (ranks[j] + 1)
...
def compute_map_and_print(dataset, ranks, gnd, kappas=[1, 5, 10]):
    ...
    if dataset.startswith('oxford5k') or dataset.startswith('paris6k') or dataset.startswith('colon'):
